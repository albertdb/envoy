...
uint64_t PrometheusStatsFormatter::statsAsPrometheus(
  ...
  for (const auto& histogram : histograms) {
    ...
    const Stats::HistogramStatistics& intervalStats = histogram->intervalStatistics();
    const std::vector<double>& supported_quantiles = intervalStats.supportedQuantiles();
    const std::vector<double>& computed_quantiles = intervalStats.computedQuantiles();
    for (size_t i = 0; i < supported_quantiles.size(); ++i) {
      double quantile = supported_quantiles[i];
      double value = computed_quantiles[i];
      response.add(fmt::format("{0}_quantile{{{1}threshold=\"{2:.3g}\"}} {3}\n", metric_name, hist_tags,
                               quantile, value));
    }
  }
